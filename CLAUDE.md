...
// Section: label_design — label_type (select: text/image, default: text), label_shape (shape-select, conditional on label_type=text, default: text-shape-badge), label_image (media, conditional on label_type=image, value: {id,url,alt} object or '')
...
// get_styles_fields() → section: label_styling — style_method (select: automatic/manual), badge_bg (color), badge_color (color), badge_radius (unit+slider), badge_font_size (unit+slider), badge_width (unit+slider, clearable=Auto, default:''), badge_height (unit+slider, clearable=Auto, default:'', no %), badge_image_width (unit+slider, default:'80px', image-type labels only), badge_padding_block (unit+slider, default:'5px'), badge_padding_inline (unit+slider, default:'14px'), badge_gap_horizontal (unit+slider, default:'10px'), badge_gap_vertical (unit+slider, default:'10px'). All fields full width (no col:half). All manual fields conditional on style_method===manual, all have css_var:true.
// get_settings_fields() → export_import section (export/import buttons), data_management section (delete_on_uninstall checkbox)
```

**`media` field type:** handled in `RenderFields.jsx` → `MediaUpload.jsx`. Opens the WP media library modal (`window.wp.media`, loaded via `wp_enqueue_media()` in `Admin::enqueue_assets`), restricted to png/jpeg/svg+xml. The preview box is also a drag-drop zone — dropped files upload via `uploadMedia()` in `api.js` (REST `POST /wp/v2/media`). **The plugin does NOT enable SVG uploads itself** — deliberate decision: SVG works only if the site already allows it (e.g. Safe SVG plugin); otherwise WP rejects the upload server-side and the field shows the error. Value stored as `{ id, url, alt }` object ('' when empty). Frontend resolves URL from attachment ID first (`wp_get_attachment_image_url`), falls back to stored `url`.
...
- `scss/index.scss` — all frontend badge styles. CSS class prefix `lpl-`. Key notes:
  ...
  - image labels: `.lpl-label--image .lpl-label__image` — `width: var(--lpl-badge-image-width, 80px)`, height auto. `width`/`max-width`/`height` need `!important` because WooCommerce's `.woocommerce-js div.product div.images img` (0,3,3) sets `width:100%` on all gallery imgs and outranks class-only selectors. Also beats the admin preview's `.lime-product-labels__preview img { width:100% }` — admin bundle imports this frontend SCSS for the live preview.
